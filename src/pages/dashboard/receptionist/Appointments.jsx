...
const Appointments = () => {
  ...
  // Tab control state: "all" displays the full database, "pending" restricts to incoming items
  const [activeTab, setActiveTab] = useState("all");
  // Sub-status filter used only when looking at the "All History" view
  const [allTabStatusFilter, setAllTabStatusFilter] = useState("all");
  ...
  const fetchAppointments = async () => {
    try {
      ...
      Swal.fire({
        ...
        text: error.response?.data?.message || "Something went wrong while loading appointments.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const updateAppointment = async (appointment, updateData, actionText) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Update Failed",
        text: error.response?.data?.message || "Something went wrong while updating appointment.",
      });
    }
  };
  ...
  const filteredAppointments = useMemo(() => {
    return appointments.filter((appointment) => {
      ...
      // Handle Tab-based status sorting logic
      let matchesStatus = false;
      if (activeTab === "pending") {
        matchesStatus = appointment.status === "pending";
      } else {
        matchesStatus = allTabStatusFilter === "all" || appointment.status === allTabStatusFilter;
      }
  ...
  }, [appointments, searchText, activeTab, allTabStatusFilter, paymentFilter]);
  ...
  if (loading) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <p className="text-lg font-black text-teal-700">Loading appointments workspace...</p>
      </div>
    );
  }
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-3xl bg-gradient-to-r from-slate-900 via-teal-800 to-cyan-700 p-6 text-white shadow-xl shadow-slate-900/20 sm:p-8">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-teal-50 sm:text-base">
          View full operational schedules, cross-examine incoming requests, filter patient issues, and alter appointment tracking states dynamically.
        </p>
      </div>

      {/* Tabs Menu Navigation inside Component */}
      <div className="flex border-b border-slate-200">
        <button
          onClick={() => setActiveTab("all")}
          className={`flex items-center gap-2 border-b-2 px-6 py-3 text-sm font-black transition-all ${
            activeTab === "all"
              ? "border-teal-700 text-teal-700 bg-teal-50/50 rounded-t-2xl"
        ...
        >
          <FaInbox />
          Pending Requests ({appointments.filter((item) => item.status === "pending").length})
        </button>
      </div>

      {/* Search and Filters Configuration Context */}
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className={`grid gap-4 ${activeTab === "all" ? "xl:grid-cols-[1fr_190px_190px]" : "xl:grid-cols-[1fr_190px]"}`}>
          ...
          {activeTab === "all" && (
            <select
              value={allTabStatusFilter}
              onChange={(e) => setAllTabStatusFilter(e.target.value)}
      ...
      {filteredAppointments.length === 0 ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            No entries match your chosen search terms or filter selections on this tab view.
          </p>
        </div>
      ) : (
              ...
              <div className="grid gap-6 xl:grid-cols-[1fr_1fr_230px]">
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm text-slate-500">
                    ...
                    <p>{appointment.patientPhone || "No phone record"}</p>
                  </div>
                  {appointment.reason && (
                    <div className="mt-4 rounded-2xl bg-slate-50 p-4">
                      ...
                      <p className="text-sm leading-6 text-slate-500">{appointment.reason}</p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  <div className="mb-4 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase tracking-wide text-slate-400">Doctor Assigned</p>
                      <h3 className="text-lg font-black text-slate-900">Dr. {appointment.doctorName}</h3>
                    </div>
                  </div>
                  <div className="space-y-1 text-sm text-slate-500">
                    <p>{appointment.doctorEmail}</p>
                    <p>{appointment.department || "General Hospital Faculty"}</p>
                    <p>{appointment.specialization || "Clinical Practice Specialist"}</p>
                  </div>
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-2xl bg-slate-50 p-4">
                      ...
                      <p className="text-sm font-bold text-slate-600">{appointment.appointmentDate}</p>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-4">
                      <div className="mb-2 flex items-center gap-2 text-sm font-black text-slate-700">
                      ...
                      <p className="text-sm font-bold text-slate-600">{appointment.appointmentTime}</p>
                    </div>
                  </div>
                </div>

                {/* Status Badges and Direct Action Controls */}
                <div className="rounded-3xl bg-slate-50 p-5">
                  <div className="space-y-3">
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-black capitalize ${statusBadgeClass(appointment.status)}`}>
                      {statusText(appointment.status)}
                    </span>
                    <div>
                      <p className="mb-2 text-xs font-black uppercase tracking-wide text-slate-400">Payment</p>
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-black capitalize ${paymentBadgeClass(appointment.paymentStatus)}`}>
                        {appointment.paymentStatus}
                      </span>
                    </div>
                    <div>
                      <p className="mb-1 text-xs font-black uppercase tracking-wide text-slate-400">Total Due</p>
                      <p className="text-xl font-black text-slate-900">৳{appointment.doctorFee || 0}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 flex flex-col gap-2">
                    {appointment.status === "pending" && (
                      <>
                        <button
                          onClick={() => updateAppointment(appointment, { status: "confirmed" }, "Confirm")}
                          className="rounded-xl bg-green-600 px-4 py-2.5 text-sm font-black text-white transition hover:bg-green-700"
                        >
                          Confirm Booking
                        </button>
                        <button
                          onClick={() => updateAppointment(appointment, { status: "rejected" }, "Reject")}
                    ...
                    {appointment.status === "confirmed" && (
                      <>
                        {appointment.paymentStatus === "paid" ? (
                          <button
                            onClick={() => updateAppointment(appointment, { paymentStatus: "unpaid" }, "Mark Unpaid")}
                          ...
                          <button
                            onClick={() => updateAppointment(appointment, { paymentStatus: "paid" }, "Mark Paid")}
                        ...
                        <button
                          onClick={() => updateAppointment(appointment, { status: "cancelled" }, "Cancel")}
                    ...
                    {(appointment.status === "rejected" || appointment.status === "cancelled") && (
                      <button
                        onClick={() => updateAppointment(appointment, { status: "pending" }, "Move to Pending")}
                        className="rounded-xl bg-yellow-100 px-4 py-2.5 text-sm font-black text-yellow-700 transition hover:bg-yellow-200"
                      >
                        Move to Pending
                      </button>
